...
# constraints
for task in remain_tasks:
    model.Add(assigned_tasks[task] == sum(task2team[(task, team)] for team in range(num_teams) \
                                                            if (task, team) in costs))
    model.Add(assigned_tasks[task] <= 1)
...
for task_i, task_j in constraints:
    if task_i in remain_tasks and task_j in remain_tasks:
        model.Add(start_task_time[task_j] + MAX_INT*(2 - assigned_tasks[task_i] - assigned_tasks[task_j]) \
                  >= start_task_time[task_i] + durations[task_i])
        
        # model.Add(assigned_tasks[task_i] >= assigned_tasks[task_j])

        
model.Maximize(sum(assigned_tasks[task] for task in remain_tasks))
